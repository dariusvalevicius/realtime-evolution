...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\dariu\\Documents\\PhD\\ImageGeneration\\repo\\realtime-evolution\\image_gen_mri.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    for thisGeneration in generations:
        ...
        onset_times = np.empty(pop_size)
        ratings = np.empty(pop_size)
        response_times = np.empty(pop_size)
        ...
        for thisTrial in trials:
            ...
            if keys is None:
                ratings[trials.thisN] = pd.NA
                response_times[trials.thisN] = pd.NA
