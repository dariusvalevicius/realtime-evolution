...
def new_generation(fitness, embeddings, iteration):
    ...
    next_embeddings = np.random.multivariate_normal(mean, mutation_size * cov, size=pop_size)
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\dariu\\Documents\\PhD\\ImageGeneration\\mri\\realtime-evolution\\image_gen_mri.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    all_onset_times = np.zeros((max_iters, pop_size))
    all_scores = np.zeros((max_iters, pop_size))
    ...
    key_resp.rt = []
    _key_resp_allKeys = []
    # Run 'Begin Routine' code from start_timer
    timer.reset()
    ...
    for thisGeneration in generations:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        # Run 'Begin Routine' code from core_code
        ...
        #        fitness = all_ratings[iteration - 1, :] # Skip dummy image
        #    elif condition == "control":
        #        fitness = np.random.uniform(size = pop_size)
        #    else:
        ...
        while continueRoutine:
            ...
            if text_2.status == STARTED:
                # update params
                pass
            # Run 'Each Frame' code from core_code
        ...
        for thisTrial in trials:
            ...
            # the Routine "washout" was not non-slip safe, so reset the non-slip timer
            routineTimer.reset()
            thisExp.nextEntry()
        ...
        while continueRoutine:
            ...
            fitness_path = f"{this_gen_output_path}/fitness.txt"
            
            if os.path.isfile(fitness_path):
                fitness = np.loadtxt(fitness_path, delimiter=',')
                try:
                    if fitness[0]:
                        continueRoutine = False
                except:
